...
class AnnotatedDocumentCleaning {
  ...
  PreprocessedDocument prepareDocument(
    ...
    return new PreprocessedDocument(
        testData.getText(),
        processCopybooks(
            collectCopybooks(explicitCopybooks, testData.getCopybookUsages(), sqlBackend, stdSqlEnabled),
            expectedDiagnostics,
            testData),
        testData);
  }
  ...
  private Stream<CobolText> collectUsedPredefinedCopybooks(
      Set<String> copybookUsages, List<String> explicitCopybooks, SQLBackend sqlBackend, boolean stdSqlEnabled) {
    return PredefinedCopybooks.getNames().stream()
        .filter(copybookUsages::contains)
        .filter(it -> !explicitCopybooks.contains(it))
        .map(PredefinedCopybookUtils.toCobolText(sqlBackend, stdSqlEnabled));
  }
