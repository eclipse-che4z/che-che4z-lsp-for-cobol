...
class VariableDefinitionUtilTest {
  ...
  void simpleStructureTest() {
    Node programNode = new ProgramNode(LOCALITY, null, null);
    // 01 Level-01-order-1
    // 01 Level-01-order-2
    //    05 Level-05-order-1
    //       10 Level-10-order-1
    //    05 Level-05-order-2
    // 01 Level-01-order-3
    programNode.addChild(VariableDefinitionNode.builder()
        .level(1)
  ...
  private static void checkNames(List<VariableNode> nodes, String... names) {
    List<String> actualNames = nodes.stream()
        .map(VariableNode::getName)
        .collect(Collectors.toList());
