...
public class PositiveTestUtility {
    ...
    /**
     * Assets DataName, procedure and program definition and references matches from the listing provided by compiler.
     * @param dataNameRefs References from the listing
     * @param rootNode root Node generated by LSP engine
     * @param fileName test file
     */
    void assetDefinitionsNReferencesFromSnap(Map<ReportSection, List<SysprintSnap>> dataNameRefs, Node rootNode, String fileName) {
        ...
        assertDataName(dataNameRefs.getOrDefault(ReportSection.DATA_NAMES, Collections.emptyList()), variableDefinitionFromLSPEngine, fileName);
        assertProcedures(dataNameRefs.getOrDefault(ReportSection.PROCEDURES, Collections.emptyList()), paragraphDefFromLSPEngine, fileName);
        assertPrograms(dataNameRefs.getOrDefault(ReportSection.PROGRAMS, emptyList()), programDefinitionFromLSPEngine, fileName);
    }

    private void assertPrograms(List<SysprintSnap> allProgRef, Multimap<String, Node> programDefinitionFromLSPEngine, String fileName) {
        allProgRef.forEach(prog -> {
            ...
            Assertions.assertTrue(progNode.isPresent(), "[" + fileName + "]:" + "Program definition not found for " + dataName);
    ...
    private void assertProcedures(
            ...
            Multimap<String, CodeBlockReference> paragraphDefFromLSPEngine, String fileName) {
                ...
                snap -> {
                    ...
                    Optional<Map.Entry<List<Location>, List<Location>>> foundElementFromLSPEngine =
                            paragraphDefFromLSPEngine.get(dataName).stream()
                                    ...
                                    .findFirst();
                    Assertions.assertTrue(foundElementFromLSPEngine.isPresent(), "[" + fileName + "]:" + "Procedure definition for " + dataName + " not found in LSP engine");
    ...
    private boolean matchParagraphDefinition(SysprintSnap snap, Map.Entry<List<Location>, List<Location>> ref) {
        if (snap.getDefinitionLocation() != null) {
            return   ref.getKey().stream()
                    .anyMatch(
                            node -> snap.getDefinitionLocation().equals(node.getRange()));
        }
        return ref.getKey().stream()
                .anyMatch(
                        node ->
                                node.getRange().getStart().getLine() + 1 == snap.getDefinedLineNo());
    }
    ...
    private void assertReferencesByProcedures(
        ...
        if (snap.getReferencesLocation() != null) {
            snap.getReferencesLocation().forEach(snapRef -> {
                boolean match = lspNodes.stream().map(Location::getRange).anyMatch(snapRef::equals);
                Assertions.assertTrue(match, "[" + fileName + "]:" + "Procedure snapReferences for " + snap.getDataName() + " not found at line no: " + snapRef);
            });
        } else {
            snap.getReferences().forEach(ref -> Assertions.assertTrue(referencesFromLSPEngine.contains(ref),
                    "[" + fileName + "]:" + "Procedure snapReferences for " + snap.getDataName() + " not found at line no: " + ref));
        }
    }

    private void assertDataName(
            List<SysprintSnap> dataNameRefs, Multimap<String, Node> variableDefinitionFromLSPEngine, String fileName) {
    ...
    private void assertSnapRangeReferencesByDataName(SysprintSnap snap, Collection<Node> nodes, String fileName) {
        ...
        Assertions.assertTrue(
                ...
                "[" + fileName + "]:" + "Data name definition for " + snap.getDataName() + " not found in LSP engine");
        ...
        foundVariableNodeInLSP.ifPresent(
                node -> {
                    ...
                    Assertions.assertEquals(
                            0,
                            unmatchedReferences.size(),
                            "[" + fileName + "]:" + "Data references are not found for "
                                    + snap.getDataName()
                                    + ", at: "
                                    + unmatchedReferences.stream()
                                    .map(Object::toString)
