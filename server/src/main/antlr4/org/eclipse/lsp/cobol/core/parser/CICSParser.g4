...
cobolCompilerDirectivesKeywords
   : ADATA | ADV | ANSI | APOST | AR | ARITH | AUTO | AWO
   | BIN | BLOCK0 | BUF | BUFSIZE
   | CBLCARD | CO | COBOL2 | COBOL3 | COMPAT | COMPILE | CP | CPP | CPSM | CS | CURR | CURRENCY
   | DATEPROC | DD | DECK | DIAGTRUNC | DLL | DP | DTR | DU | DYN | DYNAM
   | EDF | EJPD | EN | ENGLISH | EPILOG | EXP | EXPORTALL
   | FASTSRT | FLAG | FLAGSTD | FSRT
   | HOOK
   | INTDATE
   | JA | JP
   | KA
   | LANG | LC | LIB | LILIAN | LIN | LINECOUNT | LM | LONGMIXED | LONGUPPER | LU
   | MARGINS | MD | MDECK | MIG
   | NAT
   | NN
   | NOADATA | NOADV | NOALIAS | NOAWO
   | NOBLOCK0
   | NOC | NOCBLCARD | NOCICS | NOCMPR2 | NOCOMPILE | NOCPSM | NOCURR | NOCURRENCY
   | NOD | NODATEPROC | NODBCS | NODEBUG | NODECK | NODIAGTRUNC | NODLL | NODU | NODP | NODTR | NODYN | NODYNAM
   ...
   | NSEQ | NSYMBOL | NS
   | NUM | NUMPROC
   | OBJ | OPMARGINS | OPSEQUENCE | OP | OPT | OPTFILE | OUTDD
   | PFD | PGMN | PGMNAME | PROLOG
   | RENT | RMODE
   | SEQ | SEP | SHORT | SP | SPIE | SQLC | SQLCCSID | SS | SSR | SSRANGE | STD | SYSEIB | SZ
   | TERM | TRIG | TRUNC
   | UE
   | VBREF
   | WD
   ...
   | YEARWINDOW | YW
   | ZWB
   | C_CHAR | D_CHAR | E_CHAR | F_CHAR | G_CHAR | H_CHAR | I_CHAR | K_CHAR
...
cobolWord
   : IDENTIFIER | cics_only_words | db2sql_only_words | cobolCompilerDirectivesKeywords
   | ABEND | ABORT | ALL | ALWAYS | AS | ASCII | ASSOCIATED_DATA | ASSOCIATED_DATA_LENGTH | ATTACH | ATTRIBUTE
   | AUTODISPLAY
   | BACKPAGE | BINARY | BIND | BIT | BLOB | BOUNDS | BROWSE | BUT
   | CALC | CANCEL | CAPABLE | CCSVERSION | CHANGE | CHANGED | CHANNEL | CHECK | CLASS | CLOB | CLOSE_DISPOSITION
   | COBOL | CODE | COMMIT | COMMITMENT | CONNECT | CONTENTS | CONVENTION | COPIES | CORRECT | COUNT | CRUNCH | CURRENT
   | CURSOR
   | DATA | DATASTREAM | DBCLOB | DBNAME | DBNODE | DB_KEY | DC | DEQUEUE | DEST | DICTNAME | DICTNODE | DISCONNECT
   | DEFAULT | DEFAULT_DISPLAY | DEFINITION | DESTINATION | DFHRESP | DFHVALUE | DFLD | DIFFERENT
   | DISK | DOUBLE | DUMP | DUPLICATE
   | EAU | EBCDIC | ECHO | EDIT | ENDPAGE | ENDRPT | ENQUEUE | ENTER | ERASE | ESCAPE | EVENT | EXCLUSIVE | EXIT
   | EXITS | EXTENDED | EXTRANEOUS
   | FIND | FINISH | FIRST | FREE | FUNCTION_POINTER | GET
   | HEADER | HOLD
   | ID | IDENTICAL | IDMS | IGNORED | IMPLICIT | INTEGER | INTERNAL | INTERVAL | INTO | INVOKED | IN | IO
   | JOURNAL
   | KEEP | KEPT | KEYBOARD
   | LANGUAGE | LAST | LIBRARY | LINK | LIST | LOADLIB | LOCAL | LOG | LONG | LONG_DATE | LONG_TIME
   | LONGTERM | LOWER | LR | LTERM
   | MAP | MAPS | MMDDYYYY | MAX | MEMBERS | MESSAGE | MODIFIED | MODIFY
   | NAME | NAMED | NATIONAL | NATIONAL_EDITED | NATIVE | NETWORK | NEWPAGE | NEXT | NOAUTODISPLAY | NOBACKPAGE
   | NODEADLOCK | NODENAME | NODUMP | NOIO | NOLOCK | NORETURN | NORMAL | NOSPAN | NOTIFICATION | NOTIFY | NOWAIT
   | NOWRITE | NUMERIC_DATE | NUMERIC_TIME
   | OBTAIN | ODT | OFF | ONLY | ORDERLY | OUTIN | OWN | OWNER | OF | OBJECT | PAGE_INFO | PERMANENT | POINTER_32
   | PARMS | PASSWORD | PORT | POSITION | POST | PREFIX | PRINTER | PRIOR | PRIORITY | PROCEDURE | PROCESS | PROGRAM
   | PROTECTED | PTERM
   | READ | READER | READY | REAL | RECEIVED | RECURSIVE | REDISPATCH | REF | REMOTE | REMOVE | REPLY | REPORT
   | RETENTION | RETRIEVAL | RETURN | RETURNKEY | ROLLBACK | RUN_UNIT
   | SAVE | SCHEMA | SCRATCH | SCREEN | SCREENSIZE | SELECTIVE | SESSION | SHARED | SHORT | SHORT_DATE | SNAP | SPAN
   | SQL | START | STATISTICS | STATS | STGID | STORAGE | SYMBOL | SYSVERSION | SYSTEM
   | TASK | TERMINAL | TEXT | THEN | THREAD | THREAD_LOCAL | TIMEOUT | TIMER | TODAYS_DATE | TODAYS_NAME | TRANSACTION
   | TRANSFER | TRUNCATED | TYPEDEF
   | UNFORMATTED | UPDATE | UPGRADE | USAGE_MODE | USER | UTF_8
   | VERSION | VIRTUAL
   | WAIT | WITHIN
   | XCTL
