...
public class VariableDefinitionUtil {
   ...
   * <p>
   * The method do the following things: <p>
   * - delete all VariableDefinitionNode from the node <p>
   * - convert VariableDefinitionNode into appropriate VariableNode <p>
   * - build correct tree from VariableNodes and attach it to the node <p>
  ...
  public List<SyntaxError> processNodeWithVariableDefinitions(Node node) {
    Deque<VariableDefinitionNode> variableDefinitionNodes = node.getChildren().stream()
  ...
  private List<SyntaxError> processDefinition(Node rootNode, int level, Deque<VariableDefinitionNode> definitionNodes) {
    ...
    while (!definitionNodes.isEmpty()) {
      ...
      if (nextLevelNumber == level || nextLevelNumber == LEVEL_66 || nextLevelNumber == LEVEL_MNEMONIC) {
        ...
        if (convertResult.isPresent()) {
          ...
          // The variable can have nested variable definitions (like IndexItemNode), we need to collect them
          List<VariableNode> variables = variable.getDepthFirstStream()
              .filter(hasType(NodeType.VARIABLE))
              .map(VariableNode.class::cast)
              .collect(Collectors.toList());
          definitionNode.getNearestParentByType(NodeType.PROGRAM)
        ...
        Node parentFotNextLevel = Optional.ofNullable(lastVariableOnTheLevel)
            .filter(it -> it.getVariableType() == VariableType.GROUP_ITEM
                || it.getVariableType() == VariableType.MULTI_TABLE_DATA_NAME
                || definitionNode.getLevel() == LEVEL_88)
            .map(Node.class::cast)
            .orElse(rootNode);
        errors.addAll(processDefinition(parentFotNextLevel, definitionNode.getLevel(), definitionNodes));
      }
    }
    return errors;
  }
  ...
  private ResultWithErrors<VariableNode> independentDataItemMatcher(VariableDefinitionNode definitionNode) {
    ...
    StandAloneDataItemNode variable = new StandAloneDataItemNode(
  ...
  private ResultWithErrors<VariableNode> multiTableDataNameMatcher(VariableDefinitionNode definitionNode) {
    if (definitionNode.doesntHavePic()
      ...
      MultiTableDataNameNode variable = new MultiTableDataNameNode(
          definitionNode.getLocality(),
  ...
  private ResultWithErrors<VariableNode> tableDataNameMatcher(VariableDefinitionNode definitionNode) {
    if ((definitionNode.hasPic() || definitionNode.hasUsage())
        && definitionNode.hasOccurs()) {
      TableDataNameNode variable = new TableDataNameNode(
          definitionNode.getLocality(),
          definitionNode.getLevel(),
          getName(definitionNode),
          definitionNode.isGlobal(),
  ...
  private void createVariableNameNode(Node parent, VariableNameAndLocality nameAndLocality) {
    if (nameAndLocality == null || nameAndLocality.getName() == null || nameAndLocality.getLocality() == null) return;
    Node varName = new VariableDefinitionNameNode(nameAndLocality.getLocality(), nameAndLocality.getName());
  ...
  private static final Set<Integer> ALLOWED_TOP_LEVELS = ImmutableSet.of(LEVEL_01, LEVEL_66, LEVEL_77, LEVEL_88);
  ...
  private List<SyntaxError> processRenamesBoundaries(RenameItemNode variable,
                                                            GroupItemNode group,
                                                            VariableDefinitionNode definitionNode) {
    if (group == null) return ImmutableList.of(variable.getError(MessageTemplate.of(NO_STRUCTURE_BEFORE_RENAME)));
    List<VariableNode> nodesForRenaming = group.getChildren().stream()
        .flatMap(Node::getDepthFirstStream)
        .filter(hasType(NodeType.VARIABLE))
        .map(VariableNode.class::cast)
        .collect(Collectors.toList());
    List<SyntaxError> errors = new ArrayList<>();
    Integer renamesIndex = processRenamesClauseAndGetIndex(variable, definitionNode.getRenamesClause(),
        nodesForRenaming).unwrap(errors::addAll);
    Integer renamesThruIndex = processRenamesClauseAndGetIndex(variable, definitionNode.getRenamesThruClause(),
        nodesForRenaming).unwrap(errors::addAll);
  ...
  private ResultWithErrors<Integer> processRenamesClauseAndGetIndex(RenameItemNode variable,
    ...
    if (renamesIndex != -1)
      ...
      errors = ImmutableList.of(variable.getError(MessageTemplate.of(CHILD_TO_RENAME_NOT_FOUND, renamesName)));
  ...
  private List<SyntaxError> handleRedefines(VariableNode variableNode,
    ...
    variableNode.addChild(new VariableUsageNode(redefinesName, redefinesLocality, Type.GENERAL,
  ...
  private VariableWithLevelNode getPrecedingVariableForRedefine(VariableDefinitionNode definitionNode,
                                                                       String redefineName) {
    Deque<Node> siblings = new LinkedList<>(definitionNode.getParent().getChildren());
    while (!siblings.isEmpty()) {
      Node precedingNode = siblings.pollLast();
      if (precedingNode instanceof VariableWithLevelNode) {
        ...
        if (!variable.isRedefines() || variable.getName().equals(redefineName))
          return variable;
      }
    }
    return null;
