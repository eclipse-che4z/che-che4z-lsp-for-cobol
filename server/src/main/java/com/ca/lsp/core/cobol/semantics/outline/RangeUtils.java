...
public class RangeUtils {
  ...
  /**
   * Tests if innerSymbol programming construct is placed inside the outerSymbol programming construct,
   * by comparing its ranges.
  ...
  public boolean isInsideRange(Range outerRange, Range innerRange) {
    return isBefore(outerRange.getStart(), innerRange.getStart()) && isAfter(outerRange.getEnd(), innerRange.getEnd());
  }

  /**
   * Compare two document positions. The line is compared first. The character will be compared if lines are the same.
   * @param first the first position to be compared.
   * @param second the second position to be compared.
   * @return the value 0 if the argument position is equal to this position; a value less than 0 if this position
   * is located before the argument position; and a value greater than 0 if this position is located after the argument
   * position.
   */
  public int compareTo(Position first, Position second) {
    int lineComparison = Integer.compare(first.getLine(), second.getLine());
    return lineComparison == 0 ? Integer.compare(first.getCharacter(), second.getCharacter()) : lineComparison;
  }
   ...
   * @return true if and only if before position is strictly earlier in document than the after position.
   */
   ...
   * @param after a position.
   * @param before a position.
   * @return true if and only if after position is strictly later in document than the before position.
   */
