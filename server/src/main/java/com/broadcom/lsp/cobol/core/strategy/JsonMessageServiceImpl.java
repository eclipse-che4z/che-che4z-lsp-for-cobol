...
public class JsonMessageServiceImpl implements MessageService {
     ...
     *            in the passed order.
     * @return {@link ExternalizeMessage}
     */
    @Override
    public ExternalizeMessage getMessage(@NonNull String key, @NonNull Object... parameters) {
        if (this.customMessagesMap.isEmpty()) {
            throw new MessageTemplateLoadException("Please load messages from a template json file before using message service." +
        ...
        return ExternalizeMessage.builder()
    ...
    private MessageTemplate validateMessageTemplateExists(String key) {
        return this.getMessageTemplate(key).orElseGet(() -> {
    ...
    private MessageTemplate invalidMessageTemplate() {
        return new MessageTemplate(INVALID_KEY, INVALID_KEY_S_SUPPLIED_PLEASE_CHECK_THE_EXTERNALIZED_MESSAGE_FILES);
    }
    ...
    public void loadMessages(String filePath) {
        try (final BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            throw new MessageTemplateLoadException(String.format("Problems encountered while loading MessageTemplate from path '%s'.", filePath));
        }
    }
    ...
    private void validateMessageTemplateKeys(List<MessageTemplate> messageTemplateList) {
        ...
        final Set<String> toBeAddedKeys = messageTemplateList.stream().map(MessageTemplate::getKey).collect(Collectors.toSet());
    ...
    private void verifyNoKeyDuplicationIn(Set<String> toBeAddedKeys, Set<String> existingKeySet) {
        ...
        if (!intersection.isEmpty()) {
            errorMsg = String.format(KEYS_WITH_ID_S_ALREADY_EXISTS, StringUtils.joinWith(",", intersection.toArray()));
            throw new KeyAlreadyExistException(errorMsg);
        }
    }

    private void validateKeyDuplication(List<MessageTemplate> messageTemplateList) {
        List<String> duplicateKeys = messageTemplateList.stream()
        ...
        if (!duplicateKeys.isEmpty()) {
            String errorMsg = String.format(KEYS_WITH_ID_S_ALREADY_EXISTS, StringUtils.join(duplicateKeys, ","));
            throw new KeyAlreadyExistException(errorMsg);
        }
    }
