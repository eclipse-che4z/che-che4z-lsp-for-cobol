...
public class AntlrAdapter {
  ...
  private static CobolParser.ProgramUnitContext replaceWithAntlr(ProgramUnit p, CobolParser.ProgramUnitContext result) {
    org.eclipse.lsp.cobol.parser.hw.Token token = Utils.findStartToken(p).get();
    ...
    antlr.getChildren().add(new org.eclipse.lsp.cobol.parser.hw.Token(lexeme, startLine, startPos, token.getIndex(), TokenType.WHITESPACE));
  ...
  private CobolParser.ProgramUnitContext convertProgramNode(CstNode programUnit) {
    ...
    Optional<List<org.eclipse.lsp.cobol.parser.hw.Token>> endProgramName = getEndProgramName(programUnit);
    if (endProgramName.isPresent()) {
      ...
      endProgramStatementContext.start = Utils.toAntlrToken(Utils.findStartToken(programUnit).get(), charStream);
      endProgramStatementContext.stop = Utils.toAntlrToken(Utils.findStopToken(programUnit).get(), charStream);
      CobolParser.ProgramNameContext nameContext = new CobolParser.ProgramNameContext(endProgramStatementContext, 0);
      org.eclipse.lsp.cobol.parser.hw.Token nameToken = endProgramName.get().get(0);
      nameContext.start = Utils.toAntlrToken(nameToken, charStream);
      nameContext.stop = Utils.toAntlrToken((endProgramName.get().get(endProgramName.get().size() - 1)), charStream);
  ...
  private void collectTokens(CstNode su, List<CstNode> result) {
    if (su instanceof org.eclipse.lsp.cobol.parser.hw.Token) {
      result.add(su);
    }
  ...
  static Optional<List<org.eclipse.lsp.cobol.parser.hw.Token>> getEndProgramName(CstNode cstNode) {
    List<org.eclipse.lsp.cobol.parser.hw.Token> result = new ArrayList<>();
    if (!(cstNode instanceof ProgramUnit)) {
    ...
    for (CstNode n : cstNode.getChildren()) {
      if (!(n instanceof org.eclipse.lsp.cobol.parser.hw.Token) || ((org.eclipse.lsp.cobol.parser.hw.Token) n).getType() == TokenType.WHITESPACE) {
        continue;
      }
      if (nextName) {
        if (skip == 0) {
          result.add((org.eclipse.lsp.cobol.parser.hw.Token) n);
        } else {
          skip--;
        }
      }
      if ("END".equalsIgnoreCase(((org.eclipse.lsp.cobol.parser.hw.Token) n).getLexeme())) {
        nextName = true;
      }
    }
