...
public class TestCicsXctl {
  private static final String ALL_OPTIONS_VALID_ONE =
      "XCTL PROGRAM({$varOne}) COMMAREA({$varTwo}) LENGTH({$varFour}) INPUTMSG({$varFive}) INPUTMSGLEN({$varSix})";
  ...
  private static final String INVALID_ONE =
      "XCTL PROGRAM({$varOne}) {COMMAREA|error1}({$varTwo}) {CHANNEL|error1}({$varThree}) LENGTH({$varFour}) INPUTMSG({$varFive}) INPUTMSGLEN({$varSix})";
