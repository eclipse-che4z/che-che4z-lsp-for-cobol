...
public class TestCicsGetMain {
  private static final String ALL_VALID_ONE =
      "GETMAIN SET({$varOne}) FLENGTH({$varTwo}) BELOW INITIMG({$varThree}) EXECUTABLE SHARED NOSUSPEND USERDATAKEY";
  private static final String ALL_VALID_TWO =
      "GETMAIN LENGTH({$varTwo}) SHARED EXECUTABLE INITIMG({$varTwo}) NOSUSPEND USERDATAKEY SET({$varOne})";
  private static final String ALL_VALID_THREE =
      "GETMAIN CICSDATAKEY NOSUSPEND SHARED FLENGTH({$varTwo}) SET({$varOne}) EXECUTABLE INITIMG({$varTwo}) BELOW";
