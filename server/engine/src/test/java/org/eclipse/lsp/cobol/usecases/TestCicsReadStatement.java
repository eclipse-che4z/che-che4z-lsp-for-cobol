...
public class TestCicsReadStatement {
  ...
  private static final String READ_VALID_1 =
      "READ FILE({$varOne}) UNCOMMITTED INTO({$varOne}) RIDFLD({$varOne}) KEYLENGTH({$varOne}) GENERIC SYSID({$varOne}) LENGTH({$varOne}) DEBKEY EQUAL NOSUSPEND";
  ...
  private static final String READ_INVALID_2 =
      "READ {FILE|errorOne}({$varOne}) {DATASET|errorTwo}({$varOne}) RIDFLD({$varOne}) INTO({$varOne})";
  ...
  private static final String READ_INVALID_4 =
      "READ FILE({$varOne}) UNCOMMITTED {CONSISTENT|errorOne} INTO({$varOne}) RIDFLD({$varOne}) DEBKEY {RBA|errorTwo} EQUAL {GTEQ|errorThree}";
