...
public class TestCICSSoapfaultStatement {
  ...
  private static final String SOAPFAULT_ADD_1 =
      "SOAPFAULT ADD FAULTSTRING({$varOne}) FAULTSTRLEN({$varOne}) NATLANG({$varOne}) FROMCCSID({$varOne})";
  ...
  private static final String SOAPFAULT_CREATE_1 =
      "SOAPFAULT CREATE FAULTCODE({$varOne}) FAULTSTRING({$varOne}) FAULTSTRLEN({$varOne}) NATLANG({$varOne}) ROLE({$varOne}) ROLELENGTH({$varOne}) FAULTACTOR({$varOne}) FAULTACTLEN({$varOne}) DETAIL({$varOne}) DETAILLENGTH({$varOne}) FROMCCSID({$varOne})";
  ...
  private static final String SOAPFAULT_CREATE_INVALID =
      "SOAPFAULT CREATE {CLIENT|errorOne} {FAULTCODESTR|errorTwo}({$varOne}) FAULTSTRING({$varOne})";
  ...
  void testAddInvalid() {
    ...
    expectedDiagnostics.put(
        ...
        new Diagnostic(
            new Range(),
            "Exactly one option required, options are mutually exclusive: FAULTSTRING or SUBCODESTR",
            DiagnosticSeverity.Error,
            ErrorSource.PARSING.getText()));
    expectedDiagnostics.put(
        "errorTwo",
  ...
  void testCreateInvalid() {
    ...
    expectedDiagnostics.put(
        ...
        new Diagnostic(
            new Range(),
            "Exactly one option required, options are mutually exclusive: FAULTCODE, FAULTCODESTR, CLIENT, SERVER, SENDER or RECEIVER",
            DiagnosticSeverity.Error,
            ErrorSource.PARSING.getText()));
    expectedDiagnostics.put(
        "errorTwo",
