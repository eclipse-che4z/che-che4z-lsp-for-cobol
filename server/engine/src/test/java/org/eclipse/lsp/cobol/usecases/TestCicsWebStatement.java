...
public class TestCicsWebStatement {
  ...
  private static final String CONVERSE_TRANS_NOLENGTH_INVALID =
      "WEB {_CONVERSE SESSTOKEN({$varOne}) NONE GET TOLENGTH({$varOne}) USERNAME({$varFour}) INTO({$varFour}) PASSWORD({$varFour}) PATH({$varFour})|error|errorTwo|errorThree|errorFour_}";
  private static final String CONVERSE_PARSE_NOLENGTH_INVALID =
      " WEB {_PARSE URL({$varOne}) HOST({$varOne}) URLLENGTH({$varOne}) SCHEMENAME({$varOne}) PORTNUMBER({$varOne}) PATH({$varOne})"
