...
public class TestCicsWebStatement {
  ...
  private static final String WEB_AUTH =
      "BASICAUTH USERNAME({$varOne}) USERNAMELEN({$varOne}) PASSWORD({$varOne}) PASSWORDLEN({$varOne}) ";
  ...
  private static final String CONVERSE_VALID_1 =
      WEB
          + "CONVERSE SESSTOKEN({$varOne}) PATH({$varOne}) PATHLENGTH({$varOne}) GET MEDIATYPE({$varOne}) QUERYSTRING({$varOne}) QUERYSTRLEN({$varOne}) "
          + "DOCTOKEN({$varOne}) DOCDELETE EXPECT CLOSE "
          + WEB_AUTH
          + "INTO({$varOne}) TOLENGTH({$varOne}) MAXLENGTH({$varOne}) "
  ...
  private static final String EXTRACT_SERVER_VALID =
      WEB
          + "EXTRACT SCHEME({$varOne}) HOST({$varOne}) HOSTLENGTH({$varOne}) HOSTTYPE({$varOne}) HTTPMETHOD({$varOne}) METHODLENGTH({$varOne}) "
  ...
  private static final String EXTRACT_CLIENT_VALID =
      WEB
          ...
          + HOST_LEN_TYPE
          + " HTTPVERSION({$varOne}) VERSIONLEN({$varOne}) PATH({$varOne}) PATHLENGTH({$varOne}) PORTNUMBER({$varOne})";

  private static final String OPEN_VALID =
      WEB
          + "OPEN HOST({$varOne}) HOSTLENGTH({$varOne}) PORTNUMBER({$varOne}) SCHEME({$varOne}) CERTIFICATE({$varOne}) CODEPAGE({$varOne}) "
  ...
  private static final String PARSE_URL_VALID =
      WEB
          ...
          + HOST_LEN_TYPE
          + " PORTNUMBER({$varOne}) PATH({$varOne}) PATHLENGTH({$varOne}) QUERYSTRING({$varOne}) QUERYSTRLEN({$varOne})";

  private static final String WEB_READ = WEB + "READ ";
  private static final String READ_FORMFIELD_VALID =
      WEB_READ
          + "FORMFIELD({$varOne}) NAMELENGTH({$varOne}) VALUE({$varOne}) VALUELENGTH({$varOne}) CHARACTERSET({$varOne}) HOSTCODEPAGE({$varOne}) ";
  private static final String READ_QUERYPARM_VALID =
      WEB_READ
          + "QUERYPARM({$varOne}) NAMELENGTH({$varOne}) VALUE({$varOne}) VALUELENGTH({$varOne}) HOSTCODEPAGE({$varOne}) ";
  ...
  private static final String READNEXT_HTTPHEADER_VALID =
      WEB_READNEXT
          + "HTTPHEADER({$varOne}) NAMELENGTH({$varOne}) SESSTOKEN({$varOne}) VALUE({$varOne}) VALUELENGTH({$varOne})";

  private static final String RECEIVE_SERVER_BUFFER_VALID =
      WEB
          + "RECEIVE INTO({$varOne}) LENGTH({$varOne}) MAXLENGTH({$varOne}) NOTRUNCATE TYPE({$varOne}) SRVCONVERT CHARACTERSET({$varOne}) HOSTCODEPAGE({$varOne}) BODYCHARSET({$varOne}) MEDIATYPE({$varOne})";
  private static final String RECEIVE_SERVER_CONTAINER_VALID =
      WEB
          + "RECEIVE TOCONTAINER({$varOne}) TOCHANNEL({$varOne}) TYPE({$varOne}) CHARACTERSET({$varOne}) BODYCHARSET({$varOne}) MEDIATYPE({$varOne})";

  private static final String RECEIVE_BODY =
      WEB
          + "RECEIVE SESSTOKEN({$varOne}) MEDIATYPE({$varOne}) STATUSCODE({$varOne}) STATUSTEXT({$varOne}) STATUSLEN({$varOne}) ";
  private static final String RECEIVE_CLIENT_BUFFER_VALID =
      RECEIVE_BODY
          + "INTO({$varOne}) LENGTH({$varOne}) MAXLENGTH({$varOne}) NOTRUNCATE CLICONVERT BODYCHARSET({$varOne})";
  ...
  private static final String SEND_SERVER_VALID_1 =
      WEB
          + "SEND DOCTOKEN({$varOne}) NODOCDELETE MEDIATYPE({$varOne}) SRVCONVERT CHARACTERSET({$varOne}) STATUSCODE({$varOne}) STATUSTEXT({$varOne}) STATUSLEN({$varOne}) IMMEDIATE NOCLOSE";
  private static final String SEND_SERVER_VALID_2 =
      WEB
          + "SEND FROM({$varOne}) FROMLENGTH({$varOne}) CHUNKNO HOSTCODEPAGE({$varOne}) MEDIATYPE({$varOne}) SRVCONVERT CHARACTERSET({$varOne}) STATUSCODE({$varOne}) STATUSTEXT({$varOne}) STATUSLEN({$varOne}) IMMEDIATE NOCLOSE";
  private static final String SEND_SERVER_VALID_3 =
      WEB
          + "SEND CONTAINER({$varOne}) CHANNEL({$varOne}) MEDIATYPE({$varOne}) SRVCONVERT CHARACTERSET({$varOne}) STATUSCODE({$varOne}) STATUSTEXT({$varOne}) STATUSLEN({$varOne}) EVENTUAL CLOSE";

  private static final String SEND_CLIENT_VALID_1 =
      WEB
          + "SEND SESSTOKEN({$varOne}) GET PATH({$varOne}) PATHLENGTH({$varOne}) QUERYSTRING({$varOne}) MEDIATYPE({$varOne}) QUERYSTRLEN({$varOne}) DOCTOKEN({$varOne}) DOCDELETE CLICONVERT CHARACTERSET({$varOne})"
  ...
  private static final String SEND_CLIENT_VALID_2 =
      WEB
          + "SEND SESSTOKEN({$varOne}) CHUNKYES GET PATH({$varOne}) PATHLENGTH({$varOne}) QUERYSTRING({$varOne}) QUERYSTRLEN({$varOne}) CLICONVERT CHARACTERSET({$varOne}) EXPECT NOCLOSE BASICAUTH USERNAME({$varOne}) USERNAMELEN({$varOne}) PASSWORD({$varOne}) PASSWORDLEN({$varOne})";

  private static final String STARTBROWSE_FORMFIELD_VALID =
      WEB
          + "STARTBROWSE FORMFIELD({$varOne}) NAMELENGTH({$varOne}) CHARACTERSET({$varOne}) HOSTCODEPAGE({$varOne})";
  ...
  private static final String WRITE_HTTPHEADER_VALID =
      WEB
          + "WRITE HTTPHEADER({$varOne}) NAMELENGTH({$varOne}) SESSTOKEN({$varOne}) VALUE({$varOne}) VALUELENGTH({$varOne})";

  // Invalid use cases
  private static final String CLOSE_INVALID = WEB + "{CLOSE|errorOne}";
  private static final String READ_QUERYPARM_INVALID =
      WEB_READ
          + "QUERYPARM({$varOne}) NAMELENGTH({$varOne}) {SESSTOKEN|errorOne}({$varOne}) VALUE({$varOne}) VALUELENGTH({$varOne})";
  private static final String READNEXT_QUERYPARM_INVALID =
      WEB_READNEXT
          + "QUERYPARM({$varOne}) NAMELENGTH({$varOne}) {SESSTOKEN|errorOne}({$varOne}) VALUE({$varOne}) VALUELENGTH({$varOne})";
  private static final String SEND_SERVER_INVALID =
      WEB
          + "SEND DOCTOKEN({$varOne}) {FROM|errorOne}({$varOne}) FROMLENGTH({$varOne}) NODOCDELETE MEDIATYPE({$varOne}) SRVCONVERT CHARACTERSET({$varOne}) STATUSCODE({$varOne}) STATUSTEXT({$varOne}) STATUSLEN({$varOne}) IMMEDIATE NOCLOSE";
