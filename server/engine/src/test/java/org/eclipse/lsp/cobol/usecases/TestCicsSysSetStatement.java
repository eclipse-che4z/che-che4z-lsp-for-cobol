...
public class TestCicsSysSetStatement {
  ...
  private static final String AUTOINSTALL_VALID_2 =
      "SET AUTOINSTALL AIBRIDGE({$varOne}) CONSOLES({$varOne}) MAXREQS({$varOne}) PROGRAM({$varOne})";
  ...
  private static final String CONNECTION_VALID_2 =
      "SET CONNECTION({$varOne}) ACQSTATUS({$varOne}) AFFINITY({$varOne}) EXITTRACING({$varOne}) PENDSTATUS({$varOne}) CANCEL NORECOVDATA INSERVICE UOWACTION({$varOne}) ZCPTRACE";
  ...
  private static final String DB2ENTRY_VALID_2 =
      "SET DB2ENTRY({$varOne}) ACCOUNTREC({$varOne}) AUTHID({$varOne}) AUTHTYPE({$varOne}) BUSY({$varOne}) DISABLEDACT({$varOne}) ENABLESTATUS({$varOne}) PLAN({$varOne}) PLANEXITNAME({$varOne}) PRIORITY({$varOne}) SHARELOCKS({$varOne}) THREADLIMIT({$varOne}) THREADWAIT({$varOne})";
  ...
  private static final String DELETSHIPPED_VALID_2 =
      "SET DELETSHIPPED IDLE({$varOne}) INTERVALHRS({$varOne}) INTERVALMINS({$varOne}) INTERVALSECS({$varOne})";
  private static final String DISPATCHER_VALID_1 = "SET DISPATCHER";
  private static final String DISPATCHER_VALID_2 =
      "SET DISPATCHER MAXOPENTCBS({$varOne}) MAXSSLTCBS({$varOne}) MAXXPTCBS({$varOne}) MROBATCH({$varOne}) PRTYAGING({$varOne}) RUNAWAY({$varOne}) SCANDELAY({$varOne}) TIME({$varOne})";
  ...
  private static final String DSNAME_VALID_2 =
      "SET DSNAME({$varOne}) ACTION({$varOne}) AVAILABILITY({$varOne}) QUESCESTATE({$varOne}) BUSY({$varOne}) UOWACTION({$varOne})";
  ...
  private static final String FILE_VALID_2 =
      "SET FILE({$varOne}) ADD({$varOne}) BROWSE({$varOne}) WAIT CFDTPOOL({$varOne}) DELETE({$varOne}) DISPOSITION({$varOne}) DSNAME({$varOne}) EMPTYSTATUS({$varOne}) ENABLESTATUS({$varOne}) OPENSTATUS({$varOne}) EXCLUSIVE({$varOne}) KEYLENGTH({$varOne}) LOADTYPE({$varOne}) LSRPOOLNUM({$varOne}) MAXNUMRECS({$varOne}) READABLE RECORDSIZE({$varOne}) CONSISTENT RLS STRINGS({$varOne}) TABLE({$varOne}) TABLENAME({$varOne}) UPDATE({$varOne}) UPDATEMODEL({$varOne})";
  ...
  private static final String IPCONN_VALID_2 =
      "SET IPCONN({$varOne}) CONNSTATUS({$varOne}) PENDSTATUS({$varOne}) CANCEL NORECOVDATA INSERVICE BACKOUT";
  ...
  private static final String MONITOR_VALID_2 =
      "SET MONITOR COMPRESS CONVERSE DPLLIMIT({$varOne}) EXCEPT FILELIMIT({$varOne}) FREQUENCY({$varOne}) IDNTY PERF NORESRCE OFF SYNCPOINT TSQUEUELIMIT({$varOne}) URIMAPLIMIT({$varOne}) WEBSERVLIMIT({$varOne})";
  ...
  private static final String PROGRAM_VALID_2 =
      "SET PROGRAM({$varOne}) CEDF NEWCOPY DPLSUBSET JVMCLASS({$varOne}) JVMPROFILE({$varOne}) OPERATION({$varOne}) REPLICATOR JVM SHARESTATUS({$varOne}) STATUS({$varOne}) VERSION({$varOne})";
  private static final String SECDISCOVERY_VALID_1 = "SET SECDISCOVERY";
  private static final String SECDISCOVERY_VALID_2 =
      "SET SECDISCOVERY STATUS({$varOne}) CMD({$varOne}) DB2({$varOne}) DCT({$varOne}) FCT({$varOne}) HFS({$varOne}) JCT({$varOne}) PCT({$varOne}) PPT({$varOne}) PSB({$varOne}) RES({$varOne}) TST({$varOne}) USER({$varOne})";
  ...
  private static final String STATISTICS_VALID_2 =
      "SET STATISTICS ENDOFDAY({$varOne}) INTERVAL({$varOne}) RECORDING({$varOne}) RECORDNOW RESETNOW";
  private static final String SYSDUMPCODE_VALID_1 = "SET SYSDUMPCODE({$varOne})";
  private static final String SYSDUMPCODE_VALID_2 =
      "SET SYSDUMPCODE({$varOne}) ACTION({$varOne}) DAEOPTION({$varOne}) DSPLIST({$varOne}) DUMPSCOPE({$varOne}) JOBLIST({$varOne}) MAXIMUM({$varOne}) SHUTOPTION({$varOne}) SYSDUMPING({$varOne})";
  private static final String SYSTEM_VALID_1 = "SET SYSTEM";
  private static final String SYSTEM_VALID_2 =
      "SET SYSTEM AKP({$varOne}) DEBUGTOOL({$varOne}) DSALIMIT({$varOne}) DSRTPROGRAM({$varOne}) DTRPROGRAM({$varOne}) DUMPING({$varOne}) EDSALIMIT({$varOne}) FORCEQR({$varOne}) GMMTEXT({$varOne})";
  ...
  private static final String TCPIPSERVICE_VALID_2 =
      "SET TCPIPSERVICE({$varOne}) BACKLOG({$varOne}) MAXDATALEN({$varOne}) OPENSTATUS({$varOne}) URM({$varOne})";
  private static final String TDQUEUE_VALID_1 = "SET TDQUEUE({$varOne})";
  private static final String TDQUEUE_VALID_2 =
      "SET TDQUEUE({$varOne}) ATIFACILITY({$varOne}) ATITERMID({$varOne}) ATITRANID({$varOne}) ATIUSERID({$varOne}) ENABLESTATUS({$varOne}) OPENSTATUS({$varOne}) TRIGGERLEVEL({$varOne})";
  ...
  private static final String TRANSACTION_VALID_2 =
      "SET TRANSACTION({$varOne}) TRANDUMP PRIORITY({$varOne}) PURGEABLE RUNAWAY({$varOne}) SYSTEM SHUTDISABLED DISABLED TRANCLASS({$varOne}) SPECTRACE";
  ...
  void testCicsBundleInvalid() {
    testSingleError(
        BUNDLE_INVALID_1,
        "Options \"AVAILSTATUS, AVAILABLE, UNAVAILABLE, ENABLESTATUS, ENABLED, DISABLED, COPY or PHASEIN\" are mutually exclusive.");
  }
  ...
  void testCicsIpconnInvalid() {
    ...
    testSingleError(
        IPCONN_INVALID_2,
        "Options \"PURGETYPE, CANCEL, FORCECANCEL, FORCEPURGE, KILL or PURGE\" are mutually exclusive.");
  }
  ...
  void testCicsSecrecordingInvalid() {
    ...
    testSingleError(
        ...
        "Exactly one option required, options are mutually exclusive: ACTION, ADD MAXIMUM, MODIFY or REMOVE");
  }
