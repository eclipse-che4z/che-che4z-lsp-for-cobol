...
public class TestCicsSendStatement {
  ...
  private static final String SEND_CONVID_FROM_FLENGTH_CNOTCOMPL_DEFRESP_INVALID =
      "SEND CONVID({$varOne}) FROM({$varFour}) FLENGTH({$varTwo}) {CNOTCOMPL|error1} {DEFRESP|error2}";
  ...
  private static final String SEND_MAP_MAPPINGDEV_MAPSET_VALID =
      "SEND MAP({$varFour}) MAPPINGDEV({$varTwo}) SET({$varThree}) MAPSET({$varFour}) FROM({$varFive})";
  private static final String SEND_MAP_MAPPINGDEV_FROM_LENGTH_VALID =
      "SEND MAP({$varFour}) MAPPINGDEV({$varTwo}) SET({$varThree}) FROM({$varFour}) LENGTH({$varFive})";
  private static final String SEND_MAP_MAPPINGDEV_DATAONLY_MAPONLY_VALID =
      "SEND MAP({$varFour}) MAPPINGDEV({$varTwo}) SET({$varThree}) {MAPONLY|error1} {FROM|error2}({$varFive})";
  ...
  private static final String SEND_MAP_MAPPINGDEV_CURSOR_VALID =
      "SEND MAP({$varFour}) MAPPINGDEV({$varTwo}) SET({$varThree}) CURSOR({$varFour}) FROM({$varFive})";
  ...
  private static final String SEND_MAP_MAPPINGDEV_ERASEAUP_INVALID =
      "SEND MAP({$varFour}) MAPPINGDEV({$varTwo}) SET({$varThree}) {ERASE|error1} {ERASEAUP|error2} FROM({$varFive})";
  ...
  private static final String SEND_MAP_MAPPINGDEV_COMPLEX_VALID =
      "SEND MAP({$varFour}) MAPPINGDEV({$varTwo}) SET({$varThree}) MAPSET({$varFour}) FROM({$varFive}) LENGTH({$varSix}) CURSOR({$varSix}) ERASE FREEKB ALARM";
  ...
  private static final String SEND_PAGE_COMPLEX_VALID =
      "SEND PAGE RELEASE TRANSID({$varOne}) TRAILER({$varTwo}) SET({$varThree}) AUTOPAGE CURRENT OPERPURGE LAST";
  ...
  private static final String SEND_TEXT_COMPLEX_VALID =
      "SEND TEXT FROM({$varOne}) LENGTH({$varTwo}) CURSOR({$varThree}) ERASE ALARM FMHPARM({$varFour}) LDC({$varSix}) SET({$varSix}) HEADER({$varSix}) TRAILER({$varSix}) JUSFIRST ACCUM L80";
  ...
  private static final String SEND_TEXT_NOEDIT_TERMINAL_PAGING_WAIT_LAST_REQID_L80_INVALID =
      "SEND {_TEXT NOEDIT {TERMINAL|error2} {PAGING|error3} WAIT LAST REQID({$varOne}) L80|error1_}";
