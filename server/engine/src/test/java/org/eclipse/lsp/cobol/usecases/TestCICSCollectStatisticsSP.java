 ...
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=commands-collect-statistics">COLLECT STATISTICS Command</a>
 *
...
public class TestCICSCollectStatisticsSP {
    private static Stream<String> getValidOptions() {
        return Stream.of(
                ...
                "COLLECT STATISTICS SET({$varFour}) NOHANDLE LASTRESETHRS({$varFour}) LASTRESETMIN({$varFour}) LASTRESETSEC({$varFour})",
    ...
    void testCollectStatisticsResetMutualExInvalid() {
        Map<String, Diagnostic> expectedDiagnostic =
                ImmutableMap.of(
                        ...
                        new Diagnostic(
                                new Range(),
                                "Exactly one option required, options are mutually exclusive: LASTRESET or LASTRESETHRS",
                                DiagnosticSeverity.Error,
                                ErrorSource.PARSING.getText()),
                        "error2",
                        new Diagnostic(
