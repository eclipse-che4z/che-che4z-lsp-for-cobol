...
/** PUT CONTAINER (BTS) / PUT CONTAINER (CHANNEL) */
cics_put_container: PUT (cics_put_container_bts | cics_put_container_channel);
cics_put_container_bts: ((CONTAINER | ACTIVITY | FLENGTH) cics_data_value | ACQACTIVITY | PROCESS | ACQPROCESS | FROM cics_data_area  | cics_handle_response)*;
cics_put_container_channel: ((CONTAINER | CHANNEL | FROMCCSID | FROMCODEPAGE | FLENGTH) cics_data_value | FROM cics_data_area | BIT | CHAR | DATATYPE cics_cvda |
                APPEND | PREPEND | cics_handle_response)*;
