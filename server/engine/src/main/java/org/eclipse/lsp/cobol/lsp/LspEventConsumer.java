...
public abstract class LspEventConsumer {
    ...
    public void handle(LspEvent event) {
    ...
    public <T> void handle(LspQuery<T> event) {
        ...
        try {
            if (!event.getDependencies().stream().allMatch(LspEventDependency::isSatisfied)) {
                boolean isCanceled = event.getCancelConditions().stream().anyMatch(LspEventCancelCondition::shouldBeCanceled);
                if (isCanceled) {
