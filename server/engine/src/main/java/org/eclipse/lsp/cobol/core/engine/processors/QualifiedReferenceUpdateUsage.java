...
public class QualifiedReferenceUpdateUsage implements Processor<QualifiedReferenceNode> {
  ...
  public void accept(QualifiedReferenceNode node, ProcessingContext ctx) {
    ...
    Optional<DefinedAndUsedStructure> dataName =
        node.getChildren().stream()
            .filter(
                Node.hasType(NodeType.REFERENCE)
    ...
    dataName
        ...
        .filter(DefinedAndUsedStructure::isDefinitionMandatory)
        .ifPresent(
            usageNode ->
                ctx.getErrors()
                    .add(
  ...
  private void updateFunctionReference(QualifiedReferenceNode node, ProcessingContext ctx) {
    List<UsageNode> usageNodes =
        node.getChildren().stream()
            .filter(
                Node.hasType(NodeType.REFERENCE).or(Node.hasType(NodeType.FUNCTION_REFERENCE)))
        ...
        .ifPresent(
            program -> {
              for (UsageNode un : usageNodes) {
                ...
                un.setUsageType(UsageNode.UsageType.FUNCTION);
              }
            });
  }
  ...
  private void updateVariableUsage(QualifiedReferenceNode node, ProcessingContext ctx) {
    List<DefinedAndUsedStructure> variableUsageNodes =
        node.getChildren().stream()
            .filter(Node.hasType(NodeType.VARIABLE_USAGE).or(Node.hasType(NodeType.REFERENCE)))
            .map(DefinedAndUsedStructure.class::cast)
            .collect(Collectors.toList());
  ...
  private void updateVariableUsageFor(
      List<DefinedAndUsedStructure> nodes, QualifiedReferenceNode node, ProcessingContext ctx) {
    if (nodes.isEmpty()) {
      LOG.warn("Qualified reference node don't have any variable usages. {}", node);
      return;
    }
    Optional<Node> nearestProgramNode = node.getNearestParentByType(NodeType.PROGRAM);
    if (!nearestProgramNode.isPresent()) return;
    if (!symbolAccumulatorService.isVariableNameAllowed(nodes.get(0).getName(), (ProgramNode) nearestProgramNode.get())) {
      return;
    }
    ...
    for (VariableNode definitionNode : foundDefinitions) {
      ...
      for (DefinedAndUsedStructure usageNode : nodes) {
        ...
        updatedUsageType(usageNode, UsageNode.UsageType.VARIABLE);
      }
    }
    ...
    if (FigurativeConstants.FIGURATIVE_CONSTANTS.stream()
      ...
      updatedUsageType(nodes.get(0), UsageNode.UsageType.FIGURATIVE_CONSTANT);
    }
  }

  private static void updatedUsageType(DefinedAndUsedStructure usageNode, UsageNode.UsageType type) {
    if (usageNode instanceof UsageNode) {
        ((UsageNode) usageNode).setUsageType(type);
    }
  }
