...
public class LocalityMappingUtils {
  ...
  private int expandEmbeddedTokensToMatchCurrentLocality(
    ...
    String currentDocumentTokenText =
        Objects.requireNonNull(documentHierarchyLevel.getCurrent()).getToken();
    StringBuilder currentEmbeddedTokenText = new StringBuilder();
    if (currentDocumentTokenText.startsWith(currentEmbeddedTokenText.toString())) {
      ...
      for (int j = currentIndex; j < tokens.size(); j++) {
        ...
        if (currentDocumentTokenText.contentEquals(currentEmbeddedTokenText)) {
          currentIndex = j;
