...
public abstract class CICSOptionsCheckBaseUtility {
   ...
   * @param options Options checked to insert into error message
   */
  protected <E extends ParseTree> void checkHasObsoleteOptions(List<E> rules, String options) {
    if (!rules.isEmpty()) {
      rules.forEach(
              error ->
                      throwException(
                              ErrorSeverity.ERROR, getLocality(error), "Obsolete option provided: ", options));
    }
  }
