...
public class CICSPutContainerOptionsCheckUtility extends CICSOptionsCheckBaseUtility {
  ...
  public <E extends ParserRuleContext> void checkOptions(E ctx) {
    switch (ctx.getRuleIndex()) {
      case CICSParser.RULE_cics_put_container_bts:
        checkPutContainerBts((CICSParser.Cics_put_container_btsContext) ctx);
        break;
      case CICSParser.RULE_cics_put_container_channel:
        checkPutContainerChannel((CICSParser.Cics_put_container_channelContext) ctx);
        break;
      default:
        break;
    }
  ...
  private void checkPutContainerBts(CICSParser.Cics_put_container_btsContext ctx) {
    checkHasMandatoryOptions(ctx.CONTAINER(), ctx, "CONTAINER");
    checkHasMandatoryOptions(ctx.FROM(), ctx, "FROM");
    checkHasMutuallyExclusiveOptions("ACTIVITY or ACQACTIVITY or PROCESS or ACQPROCESS",
            ctx.ACTIVITY(), ctx.ACQACTIVITY(), ctx.PROCESS(), ctx.ACQPROCESS());
  }

  @SuppressWarnings("unchecked")
  private void checkPutContainerChannel(CICSParser.Cics_put_container_channelContext ctx) {
    checkHasMandatoryOptions(ctx.CONTAINER(), ctx, "CONTAINER");
    checkHasMandatoryOptions(ctx.FROM(), ctx, "FROM");
    checkHasMutuallyExclusiveOptions("BIT or DATATYPE or CHAR", ctx.BIT(), ctx.DATATYPE(), ctx.CHAR());
    checkHasMutuallyExclusiveOptions("APPEND or PREPEND", ctx.APPEND(), ctx.PREPEND());
