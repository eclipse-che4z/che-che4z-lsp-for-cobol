...
public class SourceUnitGraph implements AnalysisStateListener {
        ...
        .anyMatch(
            copyUri -> {
              try {
                ...
                String decodeUri = uri.toString().replace(" ", "%20");
                copyUri = new Uri(copyUri.toString().replace(" ", "%20"));
                return new URL(decodeUri).sameFile(new URL(copyUri.toString()));
              } catch (IOException e) {
  ...
  private void updateReferences(Uri uri, NodeV node) {
    ...
    Set<Location> updatedReferences =
        nodeV.getReferencedLocation().stream()
            .filter(loc -> !loc.getUri().equals(uri.toString()))
  ...
  public List<Uri> getCopybookUriInsideFolder(String parentFolder) {
    ...
    for (Uri copybookUri : allCopybooks) {
      try {
        Path copybookPath = Paths.get(URI.create(copybookUri.toString()));
