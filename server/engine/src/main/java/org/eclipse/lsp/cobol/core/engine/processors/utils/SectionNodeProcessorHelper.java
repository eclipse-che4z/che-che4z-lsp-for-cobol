...
public class SectionNodeProcessorHelper {
  ...
  public List<SyntaxError> processNodeWithVariableDefinitions(SectionNode sectionNode) {
    return processNodeWithVariableDefinitions((Node) sectionNode);
  }
  ...
  public List<SyntaxError> processNodeWithVariableDefinitions(FileDescriptionNode fileDescriptionNode) {
    return processNodeWithVariableDefinitions((Node) fileDescriptionNode);
  }

  private List<SyntaxError> processNodeWithVariableDefinitions(Node node) {
  ...
  private List<SyntaxError> processDefinition(
    ...
    while (!definitionNodes.isEmpty()) {
      ...
      if (nextLevelNumber == level || nextLevelNumber == LEVEL_66 || nextLevelNumber == LEVEL_MNEMONIC) {
        ...
        VariableNode variableNode = createVariableNode(definitionNode, errors);
  ...
  private static VariableNode createVariableNode(VariableDefinitionNode definitionNode, List<SyntaxError> errors) {
    switch (definitionNode.getLevel()) {
      case LEVEL_FD_SD: return FileDescriptionNode.fromDefinition(definitionNode);
      case LEVEL_66: return RenameItemNode.fromDefinition(definitionNode, errors);
      case LEVEL_77: return StandAloneDataItemNode.fromDefinition(definitionNode);
      case LEVEL_88: return ConditionDataNameNode.fromDefinition(definitionNode, errors);
      case LEVEL_MNEMONIC: return MnemonicNameNode.fromDefinition(definitionNode);
      case  LEVEL_MAP_NAME:  return MapNameNode.fromDefinition(definitionNode);
      default:
        ...
        if (definitionNode.doesntHavePic()) {
          ...
          if (definitionNode.doesntHaveOccurs() && !definitionNode.isBlankWhenZeroPresent()) {
            return GroupItemNode.fromDefinition(definitionNode);
          }
        }
        if (definitionNode.hasPic() || definitionNode.hasUsage()) {
          ...
          if (definitionNode.doesntHaveOccurs()) {
            return ElementaryItemNode.fromDefinition(definitionNode);
          }
        }
        break;
    }
