...
If you have Java on your workstation, COBOL Language Support uses the Java binaries in the PATH variable to execute. To use a different installation of Java, open the COBOL Language Support extension settings and specify the path in the **Java Home** setting. This setting can only be configured globally or in the user settings, not in the workspace settings.
