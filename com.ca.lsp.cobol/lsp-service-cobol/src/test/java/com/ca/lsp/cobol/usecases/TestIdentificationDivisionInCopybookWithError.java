...
public class TestIdentificationDivisionInCopybookWithError {
  ...
  private static final String IDDIV = "       IDENTIFICATION DIVISIONs.";
  ...
  private void assertDiagnostics(Map<String, List<Diagnostic>> diagnostics) {
    ...
    assertEquals(message, 1, diagnostics.get(IDDIV_URI).size());

    Diagnostic divisions = diagnostics.get(IDDIV_URI).get(0);

    assertEquals("Syntax error on 'DIVISIONs' expected DIVISION", divisions.getMessage());
    assertEquals(new Range(new Position(0, 22), new Position(0, 31)), divisions.getRange());
    assertEquals(Error, divisions.getSeverity());
    assertEquals("COBOL Language Support - E", divisions.getSource());
    assertNull(divisions.getCode());
  }
