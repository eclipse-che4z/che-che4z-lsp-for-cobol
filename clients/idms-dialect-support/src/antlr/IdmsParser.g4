...
idms_map_name
    : {this.validateLength(this.inputStream?.LT(1)?.text ?? "", "map name", 8);} variableUsageName
    ;

idms_db_entity_name
    : {this.validateLength(this.inputStream?.LT(1)?.text ?? "", "db entity name", 16);} variableUsageName
    ;

idms_map_name_definition
    : {this.validateLength(this.inputStream?.LT(1)?.text ?? "", "map name", 8);} dataName
    ;

idms_schema_name
    : {this.validateLength(this.inputStream?.LT(1)?.text ?? "", "schema name", 8);} dataName
    ;

idms_subschema_name
    : {this.validateLength(this.inputStream?.LT(1)?.text ?? "", "subschema name", 8);} dataName
    ;

idms_dictionary_name
    : {this.validateLengthTrimBorders(this.inputStream?.LT(1)?.text ?? "", "dictionary name", 8);} literal
    ;

idms_node_name
    : {this.validateLengthTrimBorders(this.inputStream?.LT(1)?.text ?? "", "node name", 8);} literal
    ;

idms_procedure_name
    : {this.validateLength(this.inputStream?.LT(1)?.text ?? "", "procedure name", 8);} variableUsageName
    ;

idms_table_name
    : {this.validateLengthTrimBorders(this.inputStream?.LT(1)?.text ?? "", "table name", 8);} literal
    ;
...
idms_program_name
    : {this.validateLengthTrimBorders(this.inputStream?.LT(1)?.text ?? "", "program name", 8);} literal
    ;
...
pathStatus: {this.validateLength(this.inputStream?.LT(1)?.text ?? "", "path-status", 16);} cobolWord;
