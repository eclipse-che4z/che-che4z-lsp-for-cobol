...
describe("Test Copybook code action provider", () => {
    ...
    const range = { start: { line: 4, character: 5 }, end: { line: 4, character: 6 } };
    ...
    test("code action is not triggred when diaognosis not completed", async () => {
        ...
        expect((await copybooksCodeAction.provideCodeActions(doc, range as any, context as any, token)).length).toBe(0);
    });

    test("code action is not triggred even if diaognosis is completed with code different then `missing copybook` ", async () => {
        const context = { triggerKind: {}, diagnostics: [{ range: { start: { line: 4, character: 5 }, end: { line: 4, character: 6 } }, code: "something" }], only: undefined };
        expect((await copybooksCodeAction.provideCodeActions(doc, range as any, context as any, token)).length).toBe(0);
    });

    test("code action is triggred when diaognosis completed and diaognis code is `missing copybook`", async () => {
        const context = { triggerKind: {}, diagnostics: [{ range: { start: { line: 4, character: 5 }, end: { line: 4, character: 6 } }, code: "missing copybook" }], only: undefined };
        expect((await copybooksCodeAction.provideCodeActions(doc, range as any, context as any, token)).length).toBe(1);
        expect(TelemetryService.registerEvent).toHaveBeenCalledWith("QuickFix for copybook activation", ["COBOL", "hover", "copybook", "quickfix"], "User try to understand the syntax error for a missing copybook");
    });
});
