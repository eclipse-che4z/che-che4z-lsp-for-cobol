...
describe("CustomBuildTaskTerminal Test", () => {
    ...
    it("checks proper flow of task", async () => {
        ...
        (vscode.workspace as any) = {
            openTextDocument: openDoc.mockResolvedValue({getText: jest.fn().mockReturnValue("A DUMMY DOC FOR TEST")}),
        };
        ...
        await (task as any).compileService.compileCobolDocument(Utils.getZoweExplorerAPI(), loadedProfile);
        expect(writterEvent).toBeCalledTimes(10);
    });
