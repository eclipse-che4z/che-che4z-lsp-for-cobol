...
export class CopybookURI {
    ...
    public async resolveCopybookURI(copybookName: string, cobolProgramName: string): Promise<string> {
        ...
        if (!result) {
            result = CopybookURI.searchInWorkspace(copybookName, this.createPathForCopybookDownloaded(await this.profileService.getProfileFromDocument(cobolProgramName)));
        }
