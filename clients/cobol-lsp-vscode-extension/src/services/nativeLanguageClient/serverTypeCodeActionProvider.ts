...
export class serverTypeCodeActionProvider implements vscode.CodeActionProvider {
    ...
    public async provideCodeActions(_doc: vscode.TextDocument,
        ...
        TelemetryService.registerEvent("QuickFix for server type", ["COBOL", "native server", "server type", "quickfix"], "User is trying to fix in compatible server type and dialects");

        const goToDialectsSettings = new vscode.CodeAction(QUICKFIX_UPDATE_DIALECTSSETTINGS, vscode.CodeActionKind.QuickFix);
        const goToServerTypeSettings = new vscode.CodeAction(QUICKFIX_UPDATE_SERVER_TO_JAVA, vscode.CodeActionKind.QuickFix);
