...
export class DaCoPreprocessor {
  ...
  private traverseChildren(children: StatementDescriptor[]): Item[] {
    ...
    children.forEach((child) => {
      if (child.type === "VARIABLE") {
        const tokens: Token[] = [];
        tokens.push({ name: child.name, range: child.statementRange });

        if (child.children.length > 0) {
          tokens.push(...this.createTokens(child.name, child.children));
        }
        const item: Item = {
          type: "VARIABLE",
          tokens: tokens,
  ...
  private createTokens(name: string, children: StatementDescriptor[]): Token[] {
    ...
    children.forEach((child) => {
      if (child.type === "VARIABLE") {
        const token: Token = { name: name, range: child.statementRange };
        tokens.push(token);
        if (child.children.length > 0) {
          tokens.push(...this.createTokens(child.name, child.children));
        }
